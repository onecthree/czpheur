...
PHP_METHOD(Request, __construct)
{
    ...
    zend_object* input_object = php_class_init(
        "Zpheur\\Schemes\\WebSocket\\Foundation\\InputBag",
        sizeof("Zpheur\\Schemes\\WebSocket\\Foundation\\InputBag") - 1
        // "Zpheur\\Schemes\\WebSocket\\Foundation\\ParameterBag",
        // sizeof("Zpheur\\Schemes\\WebSocket\\Foundation\\ParameterBag") - 1
    );
    do {
...
PHP_METHOD(Request, parseHeader)
{
    ...
    ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARR_P(global_server), zend_string* index, zval* value )
    {
        ...
        */
        if( strncmp(index->val, "WEBSOCKET_", 5) == 0 )
        {
...
ZEND_MINIT_FUNCTION(Zpheur_Schemes_WebSocket_Foundation_Request)
{
    zend_class_entry ce;

    INIT_NS_CLASS_ENTRY(ce, "Zpheur\\Schemes\\WebSocket\\Foundation", "Request", zpheur_schemes_websocket_foundation_request_class_method);
    zpheur_schemes_websocket_foundation_request_class_entry = zend_register_internal_class(&ce);
    zpheur_schemes_websocket_foundation_request_class_entry->ce_flags |= ZEND_ACC_ALLOW_DYNAMIC_PROPERTIES;
    zpheur_schemes_websocket_foundation_request_class_entry->create_object = create_request_object;
