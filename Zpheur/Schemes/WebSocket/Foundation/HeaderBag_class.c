...
ZEND_MINIT_FUNCTION(Zpheur_Schemes_WebSocket_Foundation_HeaderBag)
{
    zend_class_entry ce;

    INIT_NS_CLASS_ENTRY(ce, "Zpheur\\Schemes\\WebSocket\\Foundation", "HeaderBag", zpheur_schemes_websocket_foundation_headerbag_class_method);
    zpheur_schemes_websocket_foundation_headerbag_class_entry = zend_register_internal_class(&ce);

    zend_declare_property_null(zpheur_schemes_websocket_foundation_headerbag_class_entry, "headers", sizeof("headers") - 1, ZEND_ACC_PUBLIC);
