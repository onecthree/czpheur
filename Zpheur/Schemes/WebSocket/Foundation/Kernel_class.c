...
PHP_METHOD(HttpKernel, handle)
{
	...
	{		
		...
		#define ZWEBSOCKET_RESPONSE "Zpheur\\Schemes\\WebSocket\\Message\\Response"
		#define ZWEBSOCKET_RESPONSE_LEN sizeof("Zpheur\\Schemes\\WebSocket\\Message\\Response") - 1
		...
		switch( (1 << Z_TYPE_P(local_return_action)) )
		{
			...
			case MAY_BE_OBJECT:
			{
				...
				if( zend_string_equals_cstr(object->ce->name, ZWEBSOCKET_RESPONSE, ZWEBSOCKET_RESPONSE_LEN) ||
					( object->ce->parent && zend_string_equals_cstr(object->ce->parent->name,
						ZWEBSOCKET_RESPONSE, ZWEBSOCKET_RESPONSE_LEN) ) 
				)
				{
			...
			default:
				php_error_docref(NULL, E_ERROR,
					...
					middleware_class_name->val, ZWEBSOCKET_RESPONSE
				);	
			break;
		}
		...
		switch( (1 << Z_TYPE_P(local_return_action)) )
		{
			...
			default:
				...
				php_error_docref(NULL, E_ERROR,
					...
					Z_STR_P(action_class)->val, Z_STR_P(action_method)->val, ZWEBSOCKET_RESPONSE
				);	
			break;
		}
	...
	do {
		...
		switch( (1 << Z_TYPE_P(return_action)) )
		{
			...
			default:
				php_error_docref(NULL, E_ERROR,
					"%s::%s() return type must be instance of %s",
					class_src, method_src, ZWEBSOCKET_RESPONSE
				);	
			break;
		}
	} while(0); // Action method
...
ZEND_MINIT_FUNCTION(Zpheur_Schemes_WebSocket_Foundation_Kernel)
{
    zend_class_entry ce;

    INIT_NS_CLASS_ENTRY(ce, "Zpheur\\Schemes\\WebSocket\\Foundation", "Kernel", zpheur_schemes_websocket_foundation_kernel_class_method);
    zpheur_schemes_websocket_foundation_kernel_class_entry = zend_register_internal_class(&ce);
    zpheur_schemes_websocket_foundation_kernel_class_entry->ce_flags |= ZEND_ACC_ALLOW_DYNAMIC_PROPERTIES;
    zpheur_schemes_websocket_foundation_kernel_class_entry->create_object = create_kernel_object;
